...
void preorder(TreeNode* root, char** matr, int n, int m, FILE* file, int level){
    if(root==NULL) return;
    if(level!=0)
        schimba(matr, root->coordonate);
    for(int i = 0; i < n; i++) {
        for(int j = 0; j < m; j++) {
            fprintf(file, "%c", matr[i][j]);
        }
        fprintf(file, "\n");
    }
    fprintf(file, "\n");

    preorder(root->left, matr, n, m, file, level + 1);
    preorder(root->right, matr, n, m, file, level + 1);
