...
void afisare_generatie(char **matr, int n, const char *fisier_output) {
    FILE *file = fopen(fisier_output, "a");
    if (file == NULL) {
        perror("Eroare la adaugarea generatiei");
        exit(1);
    }
    for (int i = 0; i < n; i++) {
        fprintf(file, "%s\n", matr[i]);
...
int main(int argc, char **argv) {
    ...
    afisare_in_fisier(matr, n, argv[2]); 

    for (int i = 0; i < k; i++) {
        prelucrare(matr, n, m);
        afisare_generatie(matr, n, argv[2]); 
    }
